  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-heading font-bold text-brand-black dark:text-slate-50 tracking-tight">Candidate Pipeline</h2>
      ...
      <div className="flex flex-nowrap gap-4 overflow-x-auto pb-4 snap-x hide-scrollbar">
        {currentColumns.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className="flex flex-col min-w-[280px] w-[280px] shrink-0 bg-white dark:bg-slate-800/60 rounded-2xl border border-gray-200 dark:border-slate-700/80 p-3 snap-start"
            >
              {/* Column Header */}
              <div className="flex items-center justify-between px-2 py-2 mb-3 border-b border-gray-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  <span className={`h-2.5 w-2.5 rounded-full ${col.bg} border ${col.border}`} />
                  <span className="text-sm font-bold text-gray-800 dark:text-slate-100">{col.label}</span>
                </div>
                <span className="text-sm font-bold text-brand-grey bg-gray-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                  {items.length}
                </span>
              </div>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto max-h-[calc(100vh-280px)] px-1 pb-2 pt-1 -mx-1 hide-scrollbar">
                {items.length === 0 ? (
                  <div className="py-8 text-center border border-dashed border-gray-200 dark:border-slate-700 rounded-xl">
                  ...
                  items.map((applicant) => (
                    <div
                      ...
                      className="glass-card glass-card-hover rounded-xl p-3.5 space-y-3 cursor-pointer group border border-gray-100 hover:border-gray-200 dark:border-slate-700"
                    >
                      {/* Top Info */}
                      <div>
                        <div className="flex items-start justify-between gap-2">
                          <h4 className="text-sm font-bold text-brand-black dark:text-slate-50 group-hover:text-brand-red transition-colors">
                            {applicant.applicant_name}
                          </h4>
                          {applicant.resume_attachment && (
                            <span className="p-1 rounded-md bg-gray-100 dark:bg-slate-800 text-brand-red" title="Has Resume">

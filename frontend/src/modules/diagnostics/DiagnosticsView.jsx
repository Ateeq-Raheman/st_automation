...
export function DiagnosticsView({ healthData, isLoading, onRefresh, onAutoFix, isFixing }) {
  ...
  const statusIcons = {
    ...
    info: { icon: Activity, color: 'text-indigo-400', bg: 'bg-indigo-950/60 border-indigo-800/60' },
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-white tracking-tight">System Readiness & Setup</h2>
      ...
      <div className="space-y-3">
        {checks.map((check) => {
          ...
          return (
            <div
              key={check.id}
              className="glass-panel rounded-2xl p-4 sm:p-5 border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-slate-700 transition-all"
            >
